...
async function openPhotoLightbox(btn) {
  ...
  if (!xs.length) return;
  state.photoReturnFocus = btn; state.photoGallery = xs; $('#photo-lightbox').hidden = false; document.body.classList.add('no-scroll'); showPhotoAt(Math.max(0, xs.indexOf(btn))); $('#photo-lightbox-next').focus({ preventScroll: true });
  ...
  state.photoGallery = richer; showPhotoAt(at >= 0 ? at : 0);
}
