...
async function apiHealth() {
  const res = await fetchT('/api/health', { headers: { Accept: 'application/json' } });
  if (res.status !== 200 && res.status !== 503) throw new Error(`HTTP ${res.status} — GET /api/health`);
  return res.json();
}
/* Owner token: kept in this browser only, never sent anywhere but our own API.
   Guests simply never set one, so owner-only routes answer 403 for them. */
const OWNER_TOKEN_KEY = 'placeintel.ownerToken';
function ownerToken() { try { return localStorage.getItem(OWNER_TOKEN_KEY) || ''; } catch { return ''; } }
function setOwnerToken(value) { try { const v = (value || '').trim(); if (v) localStorage.setItem(OWNER_TOKEN_KEY, v); else localStorage.removeItem(OWNER_TOKEN_KEY); return true; } catch { return false; } }
function ownerHeaders() { const t = ownerToken(); return t ? { 'X-PlaceIntel-Owner': t } : {}; }
async function apiPost(path, body) {
  const res = await fetchT(path, { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json', ...ownerHeaders() }, body: JSON.stringify(body) });
  if (!res.ok) {
    let detail = '';
...
async function apiDelete(path) { const res = await fetchT(path, { method: 'DELETE', headers: { Accept: 'application/json', ...ownerHeaders() } }); if (!res.ok) throw new Error(`HTTP ${res.status} — DELETE ${path}`); return res.json(); }
...
function bindGlobal() {
  document.addEventListener('click', (e) => {
    ...
    if (e.target.closest('#owner-token-save')) return saveOwnerToken();
...
function renderSpendPolicy(sp) { if (!sp) return ''; const src = { run: ui('本次运行', 'this run'), env: ui('部署环境变量', 'deploy env'), settings: ui('已保存设置', 'saved setting'), default: ui('默认', 'default') }[sp.source] || esc(sp.source); return `<p>${ui('付费兜底', 'Paid fallback')} (SerpAPI) · ${sp.allowed ? ui('<strong>允许</strong>：免费通道失败时会花额度', '<strong>allowed</strong>: a failed free scrape may spend credits') : ui('<strong>已封锁</strong>：免费通道失败就停下，不花额度', '<strong>blocked</strong>: a failed free scrape stops instead of spending')} · ${ui('来源', 'source')} ${src}</p>`; }
function renderSystemPanel(c, h) { const s = c.settings || {}, p = c.providers || {}, f = c.feature_status || {}, r = c.runtime?.data_dir || {}, link = c.health || {}; return `<h3>${ui('系统状态', 'System Status')}</h3>${renderLanguageControls(c)}<p>${ui('推理', 'reason')} ${esc(s.reason_model)} · ${ui('译文', 'translation')} ${esc(s.translation_model)} · ${ui('默认回答', 'default answer')} ${esc(s.default_answer_language)} · ${ui('默认报告', 'default report')} ${esc(s.default_report_language || 'auto')} · ${ui('证据', 'evidence')} ${esc(s.evidence_language)} · ${ui('缓存', 'cache')} TTL ${esc(s.cache_ttl_days)} ${ui('天', 'days')}</p><p>${ui('数据目录', 'data dir')} ${r.configured ? ui('已配置', 'configured') : ui('缺失', 'missing')} · ${ui('路径', 'path')} ${r.path_visible ? ui('可见', 'visible') : ui('隐藏', 'hidden')} · ${ui('端口', 'port')} ${esc(c.runtime?.port || '—')} · ${ui('健康', 'health')} ${h?.ok ? 'ok' : ui('需检查', 'check')}</p><p>${ui('提供商状态', 'Provider status')} · ${ui('推理', 'reason')} ${esc(p.reason?.provider || 'unknown')} · ${ui('译文', 'translate')} ${esc(p.translate?.provider || 'unknown')} · ${ui('向量', 'embed')} ${esc(p.embed?.provider || 'unknown')}</p>${renderSpendPolicy(c.spend_policy)}<p>${ui('安装状态', 'Setup state')} · ${ui('推理', 'reasoning')} ${f.reasoning?.available ? 'ok' : ui('需配置', 'setup required')} · ${ui('向量', 'embedding')} ${f.embedding?.available ? 'ok' : ui('需配置', 'setup required')} · ${ui('译文', 'translation')} ${f.translation?.available ? 'ok' : ui('需配置', 'setup required')}</p><p><a href="${esc(link.cheap_url || '/api/health')}">${ui('轻量健康检查', 'cheap health')}</a> · <a href="${esc(link.deep_url || '/api/health/deep')}">${ui('深度健康检查', 'deep health')}</a></p><p id="system-danger"><strong>${ui('危险设置', 'Dangerous settings')}</strong> — ${ui('破坏性缓存/恢复操作只保留在命令行，并且需要确认。', 'destructive cache/restore actions stay in CLI and require confirmation.')}</p><p class="owner-token-row"><label for="owner-token">${ui('主人令牌', 'Owner token')}</label> <input type="password" id="owner-token" autocomplete="off" spellcheck="false" value="${esc(ownerToken())}"> <button type="button" id="owner-token-save">${ui('保存', 'Save')}</button> <span id="owner-token-status" class="mono"></span></p><p class="hint">${ui('只存在这台浏览器里，分享给朋友的密码不含它。留空即为访客。', 'Stored in this browser only; the password you share with friends does not include it. Leave empty to act as a guest.')}</p>`; }
async function toggleSystem() { const panel = $('#system-panel'); if (!panel) return; if (!panel.hidden) { panel.hidden = true; return; } panel.hidden = false; panel.innerHTML = loadingHtml(ui('读取系统状态', 'Reading system status')); try { const [c, h] = await Promise.all([apiGet('/api/config'), apiHealth()]); initLanguage(c); panel.innerHTML = renderSystemPanel(c, h); } catch (err) { panel.innerHTML = errorHtml(ui(`系统状态读取失败：${err.message}`, `Failed to read system status: ${err.message}`)); } }
function saveOwnerToken() { const input = $('#owner-token'), status = $('#owner-token-status'); if (!input || !status) return; const stored = setOwnerToken(input.value); if (!stored) { status.textContent = ui('无法写入本浏览器存储', 'could not write to browser storage'); return; } status.textContent = input.value.trim() ? ui('已保存到本浏览器', 'saved in this browser') : ui('已清除，当前为访客', 'cleared — acting as guest'); }
